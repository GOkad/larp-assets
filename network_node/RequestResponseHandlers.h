/**
 * @file RequestResponseHandlers.h
 * @brief All Request/Response handlers
 * Each device which communicates via ESP-NOW
 * Must contain this file with device specific functionality 
 * 
 * @copyright Copyright (c) 2022
 * 
 */

/**
 * @brief Handle Authentication Response
 * 
 * @param int senderDeviceType - Type of the sender device
 * @param String data - Data sent from device
 */
void handleAuthentication ( String data ) {
//   String[] dataArray = data.split(SEPARATOR);
  // TODO: parse string data

  // TODO: Handle device authentication

}
